...
# ---------------------- 앙상블 및 제출 파일 생성 함수 ----------------------
def ensemble_and_generate_submission(scores_ease, scores_slim, X, encode_user, encode_item, alpha=0.5):
    """
    ...
    for user_idx in range(n_users):
        ...
        # 앙상블 점수 계산 (가중 평균)
        final_scores = alpha * scores_ease_user + (1 - alpha) * scores_slim_user
...
if __name__ == "__main__":
    ...
    # ---------------------- 앙상블 및 제출 파일 생성 ----------------------
    alpha = 0.5  # EASE 모델의 가중치, 필요에 따라 조정 가능
