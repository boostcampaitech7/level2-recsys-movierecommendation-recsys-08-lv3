...
def ensemble_and_generate_submission(scores_ease, scores_multivae, X, encode_user, encode_item, alphas):
    ...
    for user_idx in range(n_users):
        ...
        if len(combined_items) < 10:
            ...
            mask[list(top_ease_items)] = True
            mask[list(top_multivae_items)] = True
...
def main():
    ...
    ease_model = EASE(lambda_reg)
    ease_model.train(X)
    predict_result_ease = ease_model.predict(X)

    # ---------------------- MultiVAE 모델 학습 및 예측 ----------------------
    print("Training MultiVAE model...")
    ...
    # 옵티마이저 및 손실 함수 정의
    optimizer = torch.optim.Adam(multivae_model.parameters(), lr=learning_rate)
    criterion = nn.BCELoss()

    # MultiVAE 학습
    multivae_model.train_model(train_loader, optimizer, criterion, device, epochs=epochs)

    # MultiVAE 예측
    print("Predicting with MultiVAE model...")
    predict_result_multivae = multivae_model.predict(X_dense)

    # ---------------------- 앙상블 및 제출 파일 생성 ----------------------
    print("Generating ensemble recommendations...")
    ...
    print(f"Average Recall@10: {metric}")

    # 제출 파일 저장
    submission_path = "submission.csv"
    recommendations_df.to_csv(submission_path, index=False)
    print(f"Recommendations saved to {submission_path}")
