...
# NARM 모델 클래스 (메타데이터 통합 및 Bidirectional LSTM 추가)
class NARM(nn.Module):
    def __init__(self, num_items, embedding_size, lstm_hidden_size, num_layers=2, dropout=0.3, num_genres=0, num_writers=0, num_directors=0, num_years=0):
        ...
        # 어텐션 레이어
        self.attention = nn.Linear(lstm_hidden_size * 2, 1)  # Bidirectional LSTM의 경우 hidden_size * 2
    ...
    def forward(self, sequence, meta_features=None):
        ...
        # 어텐션을 적용한 사용자 벡터 계산
        user_vector = torch.sum(lstm_out * attn_weights, dim=1)  # [batch_size, lstm_hidden_size * 2]
                ...
                for batch_idx, (user_ids, sequences, pos_items, neg_items, meta_features) in enumerate(train_loader):
                    ...
                    with autocast():
                        ...
                        if hard_neg_ratio > 0:
                            ...
                            hard_loss = F.softplus(hard_neg_scores - pos_scores).mean()
...
# 추천 목록을 생성하고 저장하는 함수 수정 (중복 제거 및 모든 사용자 포함)
...
if __name__ == "__main__":
    ...
    train_loader = DataLoader(
        ...
        prefetch_factor=4,
        collate_fn=partial(collate_fn, num_items=train_dataset.num_items, num_negatives=num_negatives)  # 수정된 부분
    )
    ...
    model.train_model(
        ...
        total_user_histories=total_user_histories,  # 수정된 부분
        user_test_items=user_test_items,            # 수정된 부분
